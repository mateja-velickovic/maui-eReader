...
booksRouter.post('/:id/notes/',   async (req, res) => {
...
booksRouter.post('/:id/comments/',   async (req, res) => {
  console.log(res.locals)
...
booksRouter.get('/:id/comments/',   async (req, res) => {
  const book = await Book.findByPk(req.params.id, {
    include: [
      {
        model: Comment,
...
booksRouter.put('/:id',   async (req, res) => {
  try {
    ...
    console.log(error)
    const message = "Le livre n'a pas pu être modifié. Merci de réessayer dans quelques instants."
    res.status(500).json({ message, data: error })
  }
})
